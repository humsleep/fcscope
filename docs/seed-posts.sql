...
-- 운영자라는 사실을 제목에 밝힌다([운영자]) — 가짜 이용자 행세는 신뢰·가이드라인 문제.
...
from me, (values
  (4, 'squad_rate',
   '[운영자] 도르트문트 팀컬러 4-2-3-1 평가 부탁드려요',
...
-- 되돌리기(필요할 때만): 운영자가 SQL 로 넣은 [운영자] 글 삭제
-- delete from community_posts where title like '[운영자]%' and author_id = (select id from auth.users where email = 'YOUR_EMAIL');
